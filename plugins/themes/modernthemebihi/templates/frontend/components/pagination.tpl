...
{if $prevUrl || $nextUrl}
	<nav aria-label="{translate|escape key="common.pagination.label"}">
		<ul class="nav justify-content-center mt-3">
			{if $prevUrl}
				<li class="nav-item">
					<a href="{$prevUrl}" class="nav-link text-success">
			...
			{/if}
			<li class="nav-item">
				<span class="nav-link disabled">{translate key="common.pagination" start=$showingStart end=$showingEnd total=$total}</span>
			</li>
			{if $nextUrl}
			<li class="nav-item">
				<a class="next nav-link text-success" href="{$nextUrl}">
