...
<div id="main-content" class="page page_register">
	...
	<div class="auth-page-container">
		<div class="register-card">
			...
			{/if}
			<h2 class="text-center mb-2" style="font-size: 1.5rem; font-weight: 700;">{translate key="user.register"}</h2>
			<p class="text-center text-muted mb-4 font-size-13">{translate key="plugins.themes.modernthemebihi.register.subtitle"}</p>
			...
			<form class="pkp_form register" id="register" method="post" action="{url op="register"}">
				...
				{if $currentContext}
					...
					{if $userCanRegisterReviewer}
						<fieldset class="reviewer mb-3">
							<legend class="font-size-15">
							...
							<div class="fields">
								<div id="reviewerOptinGroup" class="form-group optin">
									{foreach from=$reviewerUserGroups[$contextId] item=userGroup}
										{if $userGroup->getPermitSelfRegistration()}
											<label style="font-weight: normal; cursor: pointer; display: block; margin-bottom: 5px;">
				...
				<div class="buttons text-center mt-4">
					<button class="btn-primary-custom mb-3 submit" type="submit">
						{translate key="user.register"}
					</button>

					{capture assign="rolesProfileUrl"}{url page="user" op="profile" path="roles"}{/capture}
					<a class="btn-secondary-custom" href="{url page="login" source=$rolesProfileUrl}">
