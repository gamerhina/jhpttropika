...
class CustomBlockPlugin extends BlockPlugin
{
    ...
    public function getContents($templateMgr, $request = null)
    {
        ...
        $title = $customBlockTitle[$currentLocale] ?? $customBlockTitle[$contextPrimaryLocale] ?? (is_array($customBlockTitle) && !empty($customBlockTitle) ? array_values($customBlockTitle)[0] : '');
        $content = $customBlockContent[$currentLocale] ?? $customBlockContent[$contextPrimaryLocale] ?? (is_array($customBlockContent) && !empty($customBlockContent) ? array_values($customBlockContent)[0] : '');
