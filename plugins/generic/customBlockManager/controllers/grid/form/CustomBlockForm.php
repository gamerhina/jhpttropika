...
class CustomBlockForm extends Form
{
    ...
    public function initData()
    {
        ...
        if ($plugin) {
            ...
            // Fix for locale mismatch between DB and OJS 3.4
            $currentLocale = Locale::getLocale();
            if (is_array($blockTitle) && !isset($blockTitle[$currentLocale]) && !empty($blockTitle)) {
                $blockTitle[$currentLocale] = array_values($blockTitle)[0];
            }
            if (is_array($blockContent) && !isset($blockContent[$currentLocale]) && !empty($blockContent)) {
                $blockContent[$currentLocale] = array_values($blockContent)[0];
            }
